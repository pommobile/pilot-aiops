...
for (var i = 0; i < mws.length; i++) {
    ...
    var mw = mws[i];
    Log("-- MWM_CRUD_Alert mw: " + mw);  

    var description = ""; 
    if (mw.MWM_DESCRIPTIONS.length <= 4) {
      description = mw.MWM_DESCRIPTIONS;
    } else {
      var encodedString = mw.MWM_DESCRIPTIONS;
      Log("-- MWM_CRUD_Alert encodedString: " + encodedString);
      ...
      description = JSON.parse(decodeURIComponent(output)).description;
    } 
    Log("-- MWM_CRUD_Alert description: " + description);

    var startTime = new Date(mw.EOT_STARTTIME * 1000).toISOString();
    Log("-- MWM_CRUD_Alert startTime: " + startTime);

    var endTime = new Date(mw.EOT_ENDTIME * 1000).toISOString();
    Log("-- MWM_CRUD_Alert endTime: " + endTime);

    var resource = mw.FILTERSTAT;
    Log("-- MWM_CRUD_Alert filter: " + resource);
    var match = resource.match(/Identifier\s*=\s*"([^"]+)"/);
    if (match) {
       resource = match[1];
       Log("-- MWM_CRUD_Alert resource: " + resource);
    }

    var summary = description + " from " + startTime + " to " + endTime + " on " + resource + " created";
    Log("-- MWM_CRUD_Alert " + summary);

    var id = uuid();
    Log("-- MWM_CRUD_Alert id: " + id);
    ...
    var alert = {
        ...
        type: { eventType: "problem", classification: "Impact service", condition: id },
        resource: { name: resource },
        sender: { name: resource },
